...
- (TPTopBarView *)topBarView {
    if (!_topBarView) {
        ...
        _topBarView.leftItem        = [TPBarButtonItem leftBackItem:self action:@selector(leftItemTap)];
        _topBarView.centerItem      = [TPBarButtonItem centerTitleItem:self action:@selector(centerItemTap)];
        _topBarView.rightItem       = [TPBarButtonItem rightTitleItem:self action:@selector(rightItemTap)];
        _topBarView.rightItem.title = @"";
