...
- (void)setUpUserInfo:(TuyaSmartMemberModel *)user {
    ...
    if ([TuyaSmartUser sharedInstance].phoneNumber.length > 0) {
        userName = [TuyaSmartUser sharedInstance].phoneNumber;
    } else if ([TuyaSmartUser sharedInstance].email.length > 0) {
        userName = [TuyaSmartUser sharedInstance].email;
    } else {
        userName = NSLocalizedString(@"click_bind_phone", nil);
    }
