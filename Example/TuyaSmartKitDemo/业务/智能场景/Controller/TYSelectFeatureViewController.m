...
- (void)initData {
    ...
    if (_isCondition)
    {
        ...
        [[TuyaSmartSceneManager sharedInstance] getCondicationDeviceDPList:smartDevice.deviceModel.devId success:^(NSArray<TuyaSmartSceneDPModel *> *list) {
    ...
    {
        [[TuyaSmartSceneManager sharedInstance] getActionDeviceDPList:self.entityId success:^(NSArray<TuyaSmartSceneDPModel *> *list){
            for (TuyaSmartSceneDPModel *model in list)
            {
                [weakSelf_AT.dataSource addObject:[weakSelf_AT formateSceneDPModel:model]];
            }
